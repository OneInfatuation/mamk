<template>
  <div>
    <!-- 头部 -->
    <van-nav-bar
      title="我的余额"
      left-text="返回"
      right-text="余额明细"
      left-arrow
      @click-left="onClickLeft"
      @click-right="onClickRight"
    />
    <!-- 我的余额 -->
    <p style="width: 100%; height: 2rem; line-height: 2rem; padding-left: 1rem">
      我的余额 &nbsp; &nbsp;&nbsp;<span style="color: red"> 0 学习币</span>
    </p>
    <div class="money_main">
      <div v-for="(item, index) in list" :key="index" class="money_main_list">
        <p
          class="money_main_list_content"
          :class="ActiveNum == index ? 'SpanActive' : ''"
          @click="ActiveNum = index"
        >
          {{ item }}学习币
          <span style="color: red; font-size: 0.7rem; float: right"
            >￥{{ item }}</span
          >
        </p>
      </div>
    </div>
    <!-- 充值说明 -->
    <div class="money_cz">
      <p class="money_cz_title">充值说明</p>
    </div>

    <!-- 支付按钮 -->
    <p class="button">
      <input
        type="button"
        class="button_cz"
        @click="cz_btn"
        v-show="ActiveNum != -1"
        value="立即充值"
      />
      <button v-show="ActiveNum == -1" class="button_active">立即充值</button>
    </p>

    <van-popup v-model="show" position="bottom" :style="{ height: '20%' }">
      <div class="waw_popup">
        <div class="waw_popup_box">
          <h6>选择支付方式</h6>
          <van-checkbox-group v-model="result">
            <van-checkbox name="a"> <img src="data:image/png;base64,"> 支付宝</van-checkbox>
            <van-checkbox name="b"> <img src="data:image/png;base64,"> 微信</van-checkbox>
          </van-checkbox-group>
        </div>
      </div>
    </van-popup>
  </div>
</template>

<script>
export default {
  data() {
    return {
      list: [
        "30000.00",
        "20000.00",
        "10000.00",
        "8000.00",
        "5000.00",
        "3000.00",
        "0.01",
        "100.00",
      ],
      ActiveNum: -1,
      show: false, //默认隐藏
       result: [],
    };
  },
  methods: {
    onClickLeft() {
      window.history.back();
    },
    onClickRight() {
      //点击跳转余额页面
      this.$router.push("/mymoney");
    },
    cz_btn() {
      //点击立即充值
      console.log(111);
      this.show = true;
    },
  },
};
</script>

<style scoped>
.money_main {
  /* width: 100%; */
  height: 29rem;
  background: rgba(238, 238, 238, 0.788);
}
.money_main_list {
  /* width: 100%; */
  height: 3rem;
  line-height: 3rem;
  border: 1px solid transparent;
  /* background: white; */
  margin: 0.3rem 0px;
}
.money_main_list_content {
  width: 95%;
  height: 3rem;
  margin: 0.5rem auto;
  background: white;
  border: 1px solid transparent;
  border-radius: 5px;
}
.money_cz {
  width: 100%;
  height: 5rem;
}
.money_cz_title {
  width: 90%;
  height: 2rem;
  padding-left: 1rem;
  margin: 0.5rem auto;
  border-left: 2px solid red;
  border-top: 1px solid transparent;
}
.button {
  width: 100%;
  height: 3rem;
  position: fixed;
  bottom: 0px;
  left: 0px;
  z-index: 9999;
}
.button .button_cz {
  width: 100%;
  height: 3rem;
  line-height: 3rem;
  color: white;
  text-align: center;
  background: tomato;
  border: none;
}
.button_active {
  width: 100%;
  height: 3rem;
  line-height: 3rem;
  color: white;
  text-align: center;
  background: lightgray;
  border: none;
}
.SpanActive {
  border: 1px solid rgb(255, 123, 0);
  background: rgba(255, 166, 0, 0.452);
}
.van-popup--bottom {
  bottom: 3rem;
}
.waw_popup {
  width: 100%;
  height: 100%;
  display: inline-flex;
  justify-content: center;
  align-items: center;
}
.waw_popup_box {
  width: 94%;
  height: 100%;
}
.waw_popup_box h6 {
  width: 100%;
  height: 30%;
  display: inline-flex;
  align-items: center;
  font-size: 0.7rem;
  /* background: red; */
}
.waw_popup_box .van-checkbox{
  width: 100%;
  height: 30%;
  display: inline-flex;
  align-items: center;
}
.waw_popup_box img{
  width: 12%;
  height: 12%;
}
</style>