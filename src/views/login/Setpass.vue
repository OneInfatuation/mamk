<template>
  ...
  <div>
    ...
    <header>
      <div></div>
      <p>设置密码</p>
      <div @click="$router.push('/')">跳过</div>
    </header>
    <div class="dyb_header"></div>
    <!--    设置密码 -->
    <div class="box">
      <div class="init_num">
        <input type="password" placeholder="请输入密码" @change="pass" v-model="p1"/>
      </div>
      <div class="init_num">
        <input type="password" placeholder="请再次输入密码" @change="pass" v-model="p2"/>
      </div>
    </div>
    <!-- 确认登录 -->
    <div class="login">
...
export default {
  ...
  methods: {
    ...
    pass(){
        ...
        if(this.p2==''||this.p1!=this.p2){Toast('两次密码输入不一致');return false}
